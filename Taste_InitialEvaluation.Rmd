---
title: "COVID-19 Project -- Generate z scores for Taste.acc"
...
## 1.Regress Edu & Smoke 
#### ** in Gustation
```{r regSMOKE&EDU, eval=TRUE}
# Sort
regtasklist <- c(testcat)
lm_regress_p <- list()
lm_regress_c <- list()
data_reg <- data[,c(demoinfo, testcat)]
oridata_patient <- patientdata[,c("Group","NUM","AgeGroup","EDU","SMOKE",regtasklist)] 
oridata_control <- controldata[,c("Group","NUM","AgeGroup","EDU","SMOKE",regtasklist)]
...
# Regression
  ...
  lm_regress_c[[regtasklist[taskn]]] <- lm(TEST~AgeGroup+EDU+SMOKE, oridata_control)
...
## 3. Adjust overflow values
```{r adjust}
data_reg$Gus[data_reg$Gus > 5] <- 5
data_reg$Gus[data_reg$Gus < 0] <- 0
```

## 2. Generate Bootstrap DataFrame
...
```
## 3. Calculate Z scores for patients and controls
```{r prepare_data}
...
```{r export,eval=TRUE}
# save data_reg
ExcelFilePath <- paste0(SavePath,"data_taste.acc_",timsta,".csv",collapse = NULL)
write.csv(data_reg,ExcelFilePath)
# save regression parameters
ExcelFilePath <- paste0(SavePath,"Bs_taste.acc_",timsta,".csv",collapse = NULL)
write.csv(Bs,ExcelFilePath)
# save bootstrap results
ExcelFileName <- paste0("Bootstrap_taste.acc_",timsta,".csv", collapse = NULL)
ExcelFilePath <- paste0(SavePath,ExcelFileName,collapse = NULL)
write.csv(TestsBoot,ExcelFilePath)
# save z score
ExcelFilePath <- paste0(SavePath,"z_score_taste.acc_",timsta,".csv",collapse = NULL)
